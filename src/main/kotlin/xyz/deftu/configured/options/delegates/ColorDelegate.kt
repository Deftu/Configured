...
fun Configurable.color(default: Color, block: ColorDelegate.() -> Unit): ColorDelegate {
    ...
    block(delegate)
    options.add(Option(delegate.name, delegate.localizedName, delegate.description, delegate.category, delegate.localizedCategory, delegate.default, delegate.hidden, delegate.tags, OptionType.COLOR, mapOf(
        "alpha" to delegate.alpha
    ), {
        delegate.value
    }, {
        delegate.setValue(this@color, delegate::value, (it as? Color) ?: (it as? Int)?.let { Color(it) } ?: delegate.default)
    }))
    return delegate
}
