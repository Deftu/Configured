...
fun Configurable.integer(default: Int, block: IntegerDelegate.() -> Unit): IntegerDelegate {
    ...
    block(delegate)
    options.add(Option(delegate.name, delegate.localizedName, delegate.description, delegate.category, delegate.localizedCategory, delegate.default, delegate.hidden, delegate.tags, OptionType.INTEGER, mapOf(
        "min" to delegate.min,
        "max" to delegate.max
    ), {
        delegate.value
